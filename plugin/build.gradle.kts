...
dependencies {
    // Vendored baking logic + closed key derivation (the released 4.2.0 jars).
    implementation(files("libs/deviceintelligence-gradle-4.2.0.jar"))
    implementation(files("libs/deviceintelligence-baker-4.2.0.jar"))
...
tasks.jar {
    ...
    from(zipTree("libs/deviceintelligence-gradle-4.2.0.jar")) { exclude("META-INF/**") }
    from(zipTree("libs/deviceintelligence-baker-4.2.0.jar")) { exclude("META-INF/**") }
}
